...
public class RecommendationService {
    ...
    private static final Set<String> TYPES = Set.of("city", "attraction", "hotel", "restaurant");
    private static final Set<String> FEEDBACK = Set.of("click", "ignore", "save", "like");
    ...
    public List<RecommendResult> recommendForUser(long userId, String type, String cityFilter, int limit) {
        if (!TYPES.contains(type)) throw new BizException("推荐类型无效。");
    ...
    public void recordFeedback(long userId, long targetId, String targetType, String feedback) {
        if (!TYPES.contains(targetType)) throw new BizException("推荐类型无效。");
        if (!FEEDBACK.contains(feedback)) throw new BizException("推荐反馈无效。");
        jdbcTemplate.update(
