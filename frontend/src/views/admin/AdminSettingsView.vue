...
<template>
  ...
  <form class="admin-settings-form" @submit.prevent="save">
    <section><h2>地图服务</h2><label><span>高德 Web Service Key</span><input v-model="form.vite_amap_web_key" type="password" :placeholder="configured.vite_amap_web_key === 'configured' ? '已配置，留空则保持不变' : '请输入 Key'" /></label></section>
    <section><h2>小红书内容</h2><label><span>Cookie</span><textarea v-model="form.xhs_cookie" rows="4" :placeholder="configured.xhs_cookie === 'configured' ? '已配置，留空则保持不变' : '请输入 Cookie'" /></label><label><span>采集模式</span><select v-model="form.xhs_mode"><option value="service">服务采集</option><option value="tool">智能体工具</option><option value="both">双链路合并</option></select></label></section>
    <section><h2>大模型</h2><label><span>API Key</span><input v-model="form.openai_api_key" type="password" :placeholder="configured.openai_api_key === 'configured' ? '已配置，留空则保持不变' : '请输入 API Key'" /></label><label><span>服务地址</span><input v-model="form.openai_base_url" placeholder="https://api.deepseek.com" /></label><label><span>模型名称</span><input v-model="form.openai_model" placeholder="deepseek-v4-flash" /></label></section>
