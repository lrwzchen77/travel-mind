...
const comfortCharts = computed(() => [
  {
    ...
    ticks: [1, 0.8, 0.6, 0.4],
    series: [
      { key: 'train_loss', label: '训练集', color: '#287f72', values: comfortMetrics.learning_curve.map((item) => item.train_loss) },
      { key: 'test_loss', label: '测试集', color: '#d86128', values: comfortMetrics.learning_curve.map((item) => item.test_loss) },
    ],
  },
  {
    key: 'f1', title: '测试集 Macro-F1', hint: '越高越好', min: 0.6, max: 0.85,
    ticks: [0.85, 0.8, 0.7, 0.6],
    series: [{ key: 'test_f1', label: 'Macro-F1', color: '#d86128', values: comfortMetrics.learning_curve.map((item) => item.test_f1) }],
  },
]);
...
function matrixCellStyle(value, row, column) {
  ...
  return { background: row === column ? `rgba(39, 127, 114, ${alpha})` : `rgba(216, 97, 40, ${alpha})` };
}
...
<template>
  <div class="admin-command">
    <section class="admin-command-head">
      <div>
        <p>TRAVEL MIND / OPERATIONS</p>
        <h1>平台运行指挥台</h1>
    ...
    <section class="admin-metrics admin-command-metrics" aria-label="业务核心指标">
    ...
    <section class="admin-training" aria-labelledby="training-title">
          ...
          <div class="admin-live-chart">
            ...
            <div v-else class="admin-matrix-wrap">
              ...
              <div class="admin-confusion-grid" :class="{ 'is-compact': activeMatrixLabels.length <= 3 }" :style="{ gridTemplateColumns: `88px repeat(${activeMatrixLabels.length}, minmax(54px, 1fr))` }">
                <span class="admin-matrix-corner">实际类别 ↓</span>
      ...
      <div v-if="trainingModule === 'comfort'" class="admin-feedback-stats" aria-label="真实体验反馈">
        <strong>真实反馈闭环</strong><span>累计 {{ comfortFeedbackStats.total }} 条</span>
        <span>偏松 {{ comfortFeedbackStats.labels.relaxed || 0 }}</span><span>正合适 {{ comfortFeedbackStats.labels.balanced || 0 }}</span><span>太赶 {{ comfortFeedbackStats.labels.intense || 0 }}</span>
      </div>
    ...
    <section class="admin-command-actions" aria-label="运营操作入口">
