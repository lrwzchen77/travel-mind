...
<template>
  <div ref="root" class="journal-list-page">
    <PagePrologue index="08" eyebrow="旅行游记" title="把走过的路，写成自己的故事" lead="从行程生成游记，添加照片和地点，记录每一段旅程的独特记忆。" next-label="规划新行程" next-to="/map" />
    ...
    <div v-if="!loading && !journals.length" class="empty-state empty-state--card journal-empty" data-reveal style="--reveal-delay: 0.08s">
      <strong>还没有旅行游记</strong>
      <p>从一趟已有行程开始，把照片和去过的地方写成一篇游记。</p>
      <RouterLink class="btn-link btn-coral" to="/trip-history">从已有行程开始</RouterLink>
    </div>

    <div v-else class="journal-grid" data-reveal style="--reveal-delay: 0.08s" aria-live="polite">
      <RouterLink v-for="entry in journals" :key="entry.id" :to="`/journals/${entry.id}`" class="journal-card">
        ...
        <div class="journal-card-body">
          <p class="journal-card-kicker">{{ entry.destinationCity || '目的地待补充' }} · {{ updatedAt(entry.updateTime) }}</p>
          <h2>{{ entry.title || '未命名游记' }}</h2>
          <p>{{ entry.summary || '把照片和旅途片段按天整理，写成属于你的旅行故事。' }}</p>
          <p class="journal-card-meta">{{ entry.travelDays || 1 }} 天 · {{ entry.viewCount || 0 }} 次浏览</p>
        </div>
      </RouterLink>
    </div>
