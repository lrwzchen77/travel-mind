...
Travel Mind is a full-stack project. The Maven backend root aggregates `common`, `modules`, and `app`. `app/` contains HTTP APIs under `app/src/main/java/com/zkry/api`; shared infrastructure lives in `common/*`; business capabilities live in `modules/*`. `frontend/` is the Vue 3 + Vite app shell. `python-ai/` is the FastAPI service. Flyway migrations live under `app/src/main/resources/db`, and `docs/` contains design, API, agile, test, and deploy material.
...
Use `type(scope): concise action`, with types `feat`, `fix`, `docs`, `test`, `chore`, `style`, or `ci`. Pull requests should include purpose, affected modules, configuration changes, test results, and screenshots or sample API payloads when behavior changes.
